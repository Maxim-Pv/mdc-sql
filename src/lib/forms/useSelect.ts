...
export function useSelect({ value, options, onValueChange, onChange, name, onBlur, readOnlyInput = false }: Props) {
  ...
  const filtered = useMemo(() => {
    if (readOnlyInput) return options;
    const q = inputValue.trim().toLowerCase();
    if (!q) return options;
    return options.filter((o) => o.label.toLowerCase().includes(q));
  }, [options, inputValue, readOnlyInput]);
  ...
  useEffect(() => {
    ...
    if (!list) return;
    const li = list.querySelectorAll('li')[highlightIndex] as HTMLLIElement | undefined;
    if (!li) return;
