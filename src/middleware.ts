...
export default async function middleware(req: NextRequest) {
  ...
  {
    const FALLBACK_LOCALES = ['ru', 'md'] as const;
    const locales = routing.locales ? [...routing.locales] : [...FALLBACK_LOCALES]; // string[]
    ...
    if (m) {
      const locale = m[1];
      const rest = m[2] || '';
      const isAdminSub = rest.startsWith('/admin/');
      ...
      if (isAdminSub) {
        ...
        if (!token /* || !isAdmin */) {
          const login = new URL(`/${locale}/admin`, req.url);
...
function stampAttributionCookies(
  res: NextResponse,
  req: NextRequest,
  ctx: { landingPath: string; refererHdr: string; url: URL },
) {
