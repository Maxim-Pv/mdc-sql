...
export async function GET(req: NextRequest) {
  ...
  if (kind === 'events') {
    ...
    if (scope === 'upcoming') where.publishedAt = { gte: today };
    if (scope === 'past') where.publishedAt = { lt: today };

    const orderBy =
      scope === 'upcoming'
        ? [{ sortAt: 'asc' as const }, { createdAt: 'desc' as const }]
        : [{ sortAt: 'desc' as const }, { createdAt: 'desc' as const }];
  ...
  const where: any = { published: true };
  const orderBy = [{ sortAt: 'desc' as const }, { createdAt: 'desc' as const }];
